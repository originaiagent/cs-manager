...
### 報告への記載
「codexレビュー: N周実施 / 重大の残 0 / 棚上げ M件（起票先: …）」の1行を報告の「## 参照」に必ず含める。
...
**例外（唯一）**: 下記「質問する時のルール」の 13 カテゴリに該当する場合のみ `blocking_question` JSON で質問してよい。その際は `.claude/rules/reporting.md` の中断3点セットに従い、平易な要約として最上部に「どうする？ A)推奨案 B)代替案」を提示する（これは義務であり選択肢禁止の対象外）。13 カテゴリ以外で「推奨」を付けられる時点で自走判断は完了している。聞くな、実行しろ。
...
- DATABASE_URLへの直接接続やマイグレーション埋め込みは禁止（DB操作はSupabase MCP経由）
...
4. 結果（SQL出力/スクショ）を報告の「## 参照」に **生出力のまま** Markdownコードブロックで貼付（推測サマリ禁止）。
...
報告の構造（トム判断ファースト: 冒頭に🎯ゴール+✅❌チェックリスト → ■トムの判断・行動 → 技術ログは「## 参照」へ）・本文の禁止事項・中断3点セットは `.claude/rules/reporting.md` が正。

完了と判定してよい基準は `.claude/rules/verification.md` を満たすこと（テスト未完走のまま push した場合は報告と PR 本文に「テスト未検証」と明記）。

「## 参照」に必ず含めるもの:
- 自己検証: ビルド✅or❌ / 回帰✅or❌（確認ページ列挙）/ エラー✅or❌、動作確認の検証ツール（curl / /browse / /qa）・操作内容・生エビデンス【必須・推測作文禁止。UI変更で空欄は無効=Step 5に戻れ】
- 本番E2E結果（対象PRのみ。非該当は「対象外」）: URL / UI操作内容 / DB SQL生エビデンス / ロールバック検証 / クリーンアップ✅or❌
- incident-log（バグ修正時のみ必須・未記載なら差し戻し）: 発生日 / 事象 / 根本原因 / 対処 / 再発防止
- report_package（origin-policy schema 準拠）は `.claude/.session/report-<session_id>.json` に書く（会話に貼らない。Stop hook が同ファイルを機械検証する。詳細は `.claude/rules/reporting.md`）
