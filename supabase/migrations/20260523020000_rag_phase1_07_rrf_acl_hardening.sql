-- ============================================================================
-- RAG stage2 — search_knowledge_rrf ACL ハードニング (codex R3 [P0/P1])
--   - require_acl パラメータ廃止 (caller 制御の ACL バイパス除去)
--   - cs: 社内CSツールのため internal を ACL 無しでアクセス可 (dept/role 概念なし)。
--     filter_visibility で public/internal を選択。顧客向けは ['public'] を渡す。
--   - rrf_search_enabled=false で検索無効 (RPC 直叩きも空) = rollback 実効化
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.search_knowledge_rrf(
  query_embedding public.vector, query_text text,
  match_count int DEFAULT NULL, k_rrf int DEFAULT NULL,
  filter_visibility text[] DEFAULT ARRAY['public']::text[],
  filter_tenant_id uuid DEFAULT NULL, filter_channel_id uuid DEFAULT NULL,
  filter_user_id uuid DEFAULT NULL, filter_department_ids text[] DEFAULT NULL, filter_role_ids text[] DEFAULT NULL,
  relevance_boost jsonb DEFAULT '{}'::jsonb,
  embedding_model text DEFAULT NULL, embedding_modality text DEFAULT NULL, embedding_version_param text DEFAULT NULL
)
RETURNS TABLE(chunk_id uuid, article_id uuid, article_version int, content text, contextual_prefix text,
  metadata jsonb, rrf_score double precision, vector_rank int, tsvector_rank int, trgm_rank int)
...
BEGIN
  ...
  WITH filtered_chunks AS (
    ...
    WHERE kc.is_active_generation = true AND kc.status IN ('indexed','indexed_no_prefix')
      ...
      -- cs: internal は社内ツール内アクセス可 (ACL gating 無し)。public/internal は filter_visibility で選択。
  ),
  vector_ranked AS (
    SELECT fc.id AS fid, ROW_NUMBER() OVER (ORDER BY ce.embedding OPERATOR(public.<=>) query_embedding) AS rank
