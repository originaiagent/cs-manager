...
CREATE OR REPLACE FUNCTION public.search_knowledge_rrf(
  ...
  require_acl boolean DEFAULT true
)
RETURNS TABLE(
    ...
    WHERE kc.is_active_generation = true
      ...
      AND (
        ...
        OR (kc.visibility = 'internal' AND (
          require_acl = false  -- 信頼された内部ツール(cs): ACL gating 無し。visibility+tenant/channel で絞る
          OR (pg_catalog.jsonb_typeof(kc.allowed_departments)='array'
...
REVOKE ALL ON FUNCTION public.search_knowledge_rrf(public.vector, text, int, int, text[], uuid, uuid, uuid, text[], text[], jsonb, text, text, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.search_knowledge_rrf(public.vector, text, int, int, text[], uuid, uuid, uuid, text[], text[], jsonb, text, text, text, boolean) TO role_rag_search;
