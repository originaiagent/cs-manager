...
- **管理画面で実確認**: オプション一覧に2種類存在 —「**LINE Webhook転送 転送先数:1**」**月額5,500円**（0/1・未利用）と「**LINE Webhook転送 + API連携**」**月額11,000円**（0/1・未利用）。基本は **転送先数:1**、**一方向・転送専用**（LINE公式→Lステップ→外部URL）。**承認まで約3営業日**。
  - 受信転送のみなら 5,500円版で足りる見込み。**Q3(カスタムヘッダ/認証可否)が 5,500円版で不可なら「+ API連携」11,000円版が必要になる可能性**。申込前に 5,500円版の「詳細」で設定欄を実確認する。
