...
状態: **✅ codex 設計レビュー APPROVE 取得済（2026-07-23・round1→5 で収束。blocker/major 残ゼロ）**。実装は L1 実データで Case A/B 確定後（§7）。
作成: 2026-07-23 / フェーズ: `docs/roadmap.md` L1（併用開始）/ トム向け手順は `docs/lstep-line-migration-L1-tom-guide.md`
関連メモリ: `[[line-reply-send-wiring]]` `[[lstep-line-migration]]`

> ⚠️ 本ドキュメントは**設計のみ**。実装しない。L1 の実データ（転送1通）で Case A/B を確定してから、必要な分だけ実装する。
...
### 1.1 Lステップ「Webhook転送機能」既知情報（管理画面で一部実確認・2026-07-23）
出典: linestep.jp/2025/06/13, features35 LP, prtimes（Maneql）+ **トムの Lステップ管理画面スクショ**
- **管理画面で実確認**: オプション一覧に2種類 —「**LINE Webhook転送 転送先数:1**」**月額5,500円**と「**LINE Webhook転送 + API連携**」**月額11,000円**（共に 0/1・未利用）。
- **版の確定（追加調査 2026-07-23・high）**: 「+ API連携」は**逆向き**の機能（外部→Lステップを操作＝タグ/友だち情報/メッセージ送信の書き戻し）で、転送の認証強化ではない。**受信転送のみの本移行は 5,500円版で足りる**（API連携不要）。基本版は一方向・転送専用（LINE公式→Lステップ→外部URL）。
- **転送設定欄（追加調査・複数実例一致・confidence low〜medium）**: 公開情報の設定項目は「**転送先URL貼付**」＋「**転送イベントの選択(チェック)**」の2点のみ。**カスタムヘッダ/認証トークン欄は公開情報に無し**。「詳細」ボタンは設定プレビューではなく**決済(申込)ページ**へ遷移し、実設定画面は**申込・課金後にのみ**出る。
  - → **Tier2 のヘッダ token 方式は不可の可能性が高い**。分岐: **Case A(署名透過)なら改修ゼロ**／署名も来ず・ヘッダも不可なら **URL 埋め込み秘密**（§4 非スコープ・別リスク受容レビュー）へ。ヘッダ欄の有無と署名透過は**申込前にLステップサポートへ照会**（トム向け手順書に定型質問を用意）or 実測で確定。
- 申込タイミングは情報が割れる（「即時課金・即開放」説と「約3営業日承認」説）。要実確認。
- 転送内容は「LINE公式が受け取った webhook そのもの」で Lステップ固有タグは含まない（＝ペイロード範囲の話。**バイト忠実性の保証ではない**）。
- **公開情報に無い（＝実測 or サポート確認が必要）**:
  - Q1 body をバイト完全一致で転送するか、再シリアライズするか（後者なら HMAC 破綻）
  - Q2 元 `x-line-signature` を透過するか（**最重要**。ここで Case A/B が決まる）
  - Q3 転送先設定に **URL 以外の欄（カスタムヘッダ/認証トークン）** があるか（公開情報は **URL 欄のみ**。設定画面は有料オプション申込後のログイン内で非公開）
  - Q4 リトライ/タイムアウト/配信保証（「即座に送信」以外の記載なし）
- ⚠️ **重要な訂正**: 「Lステップ転送で署名が壊れる」という説は**検索エンジンの推測合成**であり確定事実ではない。**Case A（署名透過）も十分あり得る**。Case B を前提に作らない。
...
|---|---|---|---|
| **A 素通し** | raw body バイト一致 **かつ** `x-line-signature` 無改変付与 | Tier1 検証**成立 → 200**（同 OA channel secret を Core 取得済み前提） | **改修ゼロ** |
