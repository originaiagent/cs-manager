...
| ツール間直接連携（Core非経由） | cs-manager → ec-manager `/api/external/*`（`EC_MANAGER_API_URL`+`EC_MANAGER_API_KEY`）: `sales-units`(期間販売数)・`customer-returns`(FBA返品)・`order-dates`(Amazon注文日)・`rakuten-order-items`(楽天注文番号→商品管理番号、不良率の製品特定用)。cs-manager 側は `CLAUDE.md` の `EC_MANAGER_API_URL` 記載が正 |
