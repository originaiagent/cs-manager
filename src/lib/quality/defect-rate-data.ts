 ...
 *
 * 追加 (C3b): ?view=all|factory (表示切替) と ?basis=occurred|ordered (集計基準) を解決。
...
import { DEFECT_VIEWS, type DefectView } from '@/lib/quality/defect-view';
...
/** ?view= の解決 (不正値は既定 'all') */
export function resolveView(raw: string | undefined): DefectView {
  return (DEFECT_VIEWS as readonly string[]).includes(raw ?? '') ? (raw as DefectView) : 'all';
}
