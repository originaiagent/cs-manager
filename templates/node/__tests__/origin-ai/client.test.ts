...
/**
 * Origin-AI Client Test
 * 
 * Note: These tests assume a Jest-like environment where global.fetch can be mocked.
 * If using Node 18+ native fetch, you might need a different mocking strategy 
 * (like undici or nock), but for simplicity and consistency with Next.js 
 * we use the jest.fn() pattern.
 */

// Mock fetch if it's not already a mock (or if we are in a jest environment)
