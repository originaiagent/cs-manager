...
async function requestWithRetry(
  ...
  while (true) {
    ...
    if (options.signal) {
      // In Node 18+ or modern browsers, we can use AbortSignal.any()
      // For compatibility, we might need a workaround or just hope for the best.
      // Next.js usually targets modern environments.
 ...
 *  - レスポンス field 名 (message / structured_output / skill_used) も同様。
...
export async function invokeChat(
  ...
  // snake_case / camelCase 両対応 (origin-ai 側の最終形が Phase 7 で確定)
  return {
 ...
 *
 * Phase 7 契約依存:
 *  - endpoint path (/api/managed-agent/run) と payload shape
 *    ({ workflow_id, data }) は bf9cc6e2 完了後に最終確認・調整が必要。
 *  - 現状の origin-ai 実装は agent_name + message を受けるが、
 *    invocation-pattern guide (SoT) は workflow_id + data を規定している。
 *    本配管は guide SoT 準拠の payload で送出する。
 */
